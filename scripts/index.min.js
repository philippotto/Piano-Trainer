...
(function (root, factory) {
    ...
    (function (window, document, Chartist) {
        ...
        Chartist.createSvg = function (query, width, height) {
            var container = query.nodeType ? query : document.querySelector(query), svg;
            if (!container) {
        ...
        Chartist.getBounds = function (svg, normalizedData, options, referenceValue) {
            var i, newMin, newMax, bounds = Chartist.getHighLow(normalizedData);
        ...
        Chartist.createXAxis = function (chartRect, data, grid, labels, options) {
            data.labels.forEach(function (value, index) {
                var interpolatedValue = options.axisX.labelInterpolationFnc(value, index), pos = chartRect.x1 + chartRect.width() / data.labels.length * index;
                if (!interpolatedValue && interpolatedValue !== 0) {
                    return;
                }
                if (options.axisX.showGrid) {
                    grid.elem('line', {
                        ...
                        options.classNames.horizontal
                    ].join(' '));
                }
                if (options.axisX.showLabel) {
                    var label = labels.elem('text', { dx: pos + 2 }, [
                            options.classNames.label,
                            options.classNames.horizontal
                        ].join(' ')).text('' + interpolatedValue);
                    label.attr({ dy: chartRect.y1 + Chartist.getHeight(label._node) + options.axisX.offset });
                }
            });
        };
        Chartist.createYAxis = function (chartRect, bounds, grid, labels, offset, options) {
            bounds.values.forEach(function (value, index) {
                var interpolatedValue = options.axisY.labelInterpolationFnc(value, index), pos = chartRect.y1 - chartRect.height() / bounds.values.length * index;
                if (!interpolatedValue && interpolatedValue !== 0) {
                    return;
                }
        ...
        Chartist.optionsProvider = function (defaultOptions, options, responsiveOptions, optionsChangedCallbackFnc) {
            var baseOptions = Chartist.extend(Chartist.extend({}, defaultOptions), options), currentOptions, mediaQueryListeners = [], i;
    ...
    (function (window, document, Chartist) {
        ...
        Chartist.svg = function (name, attributes, className, parent) {
            ...
            function addClass(node, names) {
                node.setAttribute('class', classes(node).concat(names.trim().split(/\s+/)).filter(function (elem, pos, self) {
    ...
    (function (window, document, Chartist) {
        ...
        Chartist.Line = function (query, data, options, responsiveOptions) {
            ...
            function createChart(options) {
                var xAxisOffset, yAxisOffset, seriesGroups = [], bounds, normalizedData = Chartist.normalizeDataArray(Chartist.getDataArray(data), data.labels.length);
                ...
                if (options.axisX.showLabel) {
                    xAxisOffset += Chartist.calculateLabelOffset(svg, data.labels, [
                        options.classNames.label,
                        options.classNames.horizontal
                    ].join(' '), options.axisX.labelInterpolationFnc, Chartist.getHeight);
                }
                yAxisOffset = options.axisY.offset;
                if (options.axisY.showLabel) {
                    yAxisOffset += Chartist.calculateLabelOffset(svg, bounds.values, [
                        options.classNames.label,
                        options.classNames.horizontal
                    ].join(' '), options.axisY.labelInterpolationFnc, Chartist.getWidth);
                }
                var chartRect = Chartist.createChartRect(svg, options, xAxisOffset, yAxisOffset);
                var labels = svg.elem('g'), grid = svg.elem('g');
                ...
                for (var i = 0; i < data.series.length; i++) {
                    ...
                    var p = Chartist.projectPoint(chartRect, bounds, normalizedData[i], 0), pathCoordinates = [
                            p.x,
                            p.y
                        ], point;
                    ...
                    if (options.showLine) {
                        ...
                        seriesGroups[i].elem('path', { d: svgPathString }, options.classNames.line);
                    }
                }
            }
    ...
    (function (window, document, Chartist) {
        ...
        Chartist.Bar = function (query, data, options, responsiveOptions) {
            ...
            function createChart(options) {
                ...
                var chartRect = Chartist.createChartRect(svg, options, xAxisOffset, yAxisOffset);
                var labels = svg.elem('g'), grid = svg.elem('g'), zeroPoint = Chartist.projectPoint(chartRect, bounds, [0], 0);
                Chartist.createXAxis(chartRect, data, grid, labels, options);
                Chartist.createYAxis(chartRect, bounds, grid, labels, yAxisOffset, options);
                for (var i = 0; i < data.series.length; i++) {
                    var biPol = i - (data.series.length - 1) / 2, periodHalfWidth = chartRect.width() / normalizedData[i].length / 2;
                    seriesGroups[i] = svg.elem('g');
                    seriesGroups[i].addClass([
                        options.classNames.series,
                        data.series[i].className || options.classNames.series + '-' + Chartist.alphaNumerate(i)
                    ...
                    for (var j = 0; j < normalizedData[i].length; j++) {
                        var p = Chartist.projectPoint(chartRect, bounds, normalizedData[i], j), bar;
    ...
    (function (window, document, Chartist) {
        ...
        Chartist.Pie = function (query, data, options, responsiveOptions) {
            ...
            function createChart(options) {
                var seriesGroups = [], chartRect, radius, totalDataSum, startAngle = options.startAngle, dataArray = Chartist.getDataArray(data);
                svg = Chartist.createSvg(query, options.width, options.height);
                ...
                for (var i = 0; i < data.series.length; i++) {
                    ...
                    var start = Chartist.polarToCartesian(center.x, center.y, radius, startAngle - (i === 0 ? 0 : 0.2)), end = Chartist.polarToCartesian(center.x, center.y, radius, endAngle), arcSweep = endAngle - startAngle <= 180 ? '0' : '1', d = [
                            'M',
                            end.x,
                            end.y,
                            'A',
                    ...
                    var path = seriesGroups[i].elem('path', { d: d.join(' ') }, options.classNames.slice + (options.donut ? ' ' + options.classNames.donut : ''));
                    if (options.donut === true) {
                        path.attr({ 'style': 'stroke-width: ' + +options.donutWidth + 'px' });
                    }
                    startAngle = endAngle;
                }
            }
